  ...
  const handleFile = useCallback((file: File) => {
    ...
    Papa.parse<CsvRow>(file, {
      ...
      complete: (result) => {
        ...
        if (!validation.ok) {
          reject(`Missing required column${validation.missing.length > 1 ? "s" : ""}: ${validation.missing.join(", ")}`);
          return;
        }
      ...
      <DialogContent className="max-w-2xl border-border bg-card">
        ...
        {parsed && (
          <div className="space-y-3">
            <div className="flex items-center justify-between rounded-md border border-healthy/30 bg-healthy/10 px-3 py-2 text-[12px] text-healthy">
                  ...
                  <TableHeader className="sticky top-0 bg-surface">
                    <TableRow className="border-border hover:bg-transparent">
                      {REQUIRED_CSV_HEADERS.map((h) => (
                        <TableHead
                          key={h}
                          className="whitespace-nowrap text-[10px] uppercase tracking-[0.1em]"
                        >
                          {h}
                        </TableHead>
                      ))}
